...
with open('0Preprocessing_data/jAER_100^2_all_imgs_betterlables/jAER_simplest_by_hand_better-targetLocations.txt') as fp:
    # Get the first line
    line = fp.readline()
    while line:
        wholeLine = line.strip()
        if ('#' != wholeLine[0]) and ('A' != wholeLine[0]):
            ...
            save(f'0Preprocessing_data/jAER_100^2_all_imgs_betterlables/labels/label-{count}.npy', frame)
            count += 1
